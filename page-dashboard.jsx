...
function DashboardPage() {
  ...
  return (
    <TelemetryChrome active="DASHBOARD">
      ...
      {/* Stations — PB Verona 2025 */}
      <ModulePanel code="MOD.STATIONS · pb_verona_2025 · 9 stations" accent>
        <div>
          {STATIONS.map((s, i) => {
            return (
              <div key={s.code} className="r-station-row" style={{
                ...
                <div className="rs-code display" style={{ fontSize: 14, color: 'var(--accent)' }}>{s.code}</div>
                <div className="rs-name" style={{ fontSize: 13, fontWeight: 500, fontFamily: 'var(--sans)' }}>{s.name}</div>
                <div className="rs-pb">
                  <div style={{ fontSize: 9, color: 'var(--fg-3)', letterSpacing: '0.15em' }}>PB VERONA</div>
