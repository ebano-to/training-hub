...
function AgendaPage() {
  ...
  const allWeeks = (WEEK_ARCHIVE || []).concat([{ id: 'S' + ATHLETE.programWeek, label: 'S' + ATHLETE.programWeek + ' · CORRENTE', range: '31 AGO → 06 SET 2026', programWeek: ATHLETE.programWeek, days: WEEK }]).concat(NEXTWEEK && NEXTWEEK.length ? [{ id: 'S' + (ATHLETE.programWeek + 1), label: 'S' + (ATHLETE.programWeek + 1) + ' · DA DEFINIRE', range: '07 → 13 SET 2026', programWeek: ATHLETE.programWeek + 1, days: NEXTWEEK }] : []);
            ...
            return (
              <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', height: 80, marginBottom: 12 }}>
                {currentWeekDays.map((w, i) => {
                  ...
                  var skipCount = blocks.filter(function(bl) { return bl.skipped || bl.result === 'SALTATO'; }).length;
                  var doneCount = w.done ? total - skipCount : blocks.filter(function(bl) { return bl.result && bl.result !== 'da fare' && bl.result !== 'SALTATO' && !bl.skipped; }).length;
                  var pendingCount = total - doneCount - skipCount;
                  var barH = maxDur > 0 ? (w.duration / maxDur) * 100 : 0;
                  return (
                  <div key={i} style={{
                    flex: 1, height: barH + '%',
                    border: (w.done && pendingCount === 0) ? '1.5px solid var(--accent)' : '1px solid var(--line)',
                    minHeight: 4, overflow: 'hidden', display: 'flex', flexDirection: 'column',
                  }}>
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                      <div style={{ flexGrow: pendingCount, background: parseInt(w.date,10) === todayNum && doneCount === 0 && skipCount === 0 ? 'transparent' : 'oklch(50% 0 0 / 0.3)' }} />
                      {skipCount > 0 && <div style={{ flexGrow: skipCount, background: 'oklch(55% 0.12 15 / 0.3)' }} />}
                  ...
                  <div key={i} style={{
                    ...
                    <div className="r-block-row" style={{
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: 'var(--fg-2)', marginTop: 3, whiteSpace: 'pre-line' }}>{b.d}</div>
                      </div>
